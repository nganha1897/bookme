...
import { useLazyUpdateSessionQuery, useUpdateUserMutation } from '@/redux/api/userApi';
...
const UpdateUser = ({ data }: Props) => {
  ...
  const [updateSession, { data: dataSession }] = useLazyUpdateSessionQuery();
  
  if (dataSession) 
    dispatch(setUser(dataSession?.user));
